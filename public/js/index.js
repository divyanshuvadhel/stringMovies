...
featuredMovies.forEach((movie, index) => {
  ...
  div.innerHTML = `
    <img src="${movie.image}" alt="${movie.title}" class="w-full h-full object-cover bg-cover">
...
topPicks.forEach((movies,index)=>{
    ...
    div.innerHTML=` <img src="${movies.image9_16}" class="w-full h-full object-contain" alt="">
                <div class="absolute top-0 h-full w-full p-3 ">
                  <span class="text-2xl font-semibold absolute bottom-10 -translate-y-1/2" id="title">${movies.title}</span>
                  <span class="absolute top-3 right-5 backdrop-blur-2xl px-3 py-1 rounded-md" id="rating"">⭐ ${movies.rating}</span>
                  <button class="absolute bottom-3  backdrop-blur-md bg-white/10   px-3 py-1 rounded-md" id="btn"> 
                    <a href="${movies.detailsLink}">view details</a>
...
newReleases.forEach((movies,index)=>{
     const div=document.createElement('div');
    div.id=index;
    div.className="relative min-w-[220px] min-h-[300px] h-full bg-center rounded-2xl overflow-hidden hover:scale-105 transition-all duration-300"
    div.innerHTML=` <img src="${movies.image9_16}" class="w-full h-full object-contain" alt="">
